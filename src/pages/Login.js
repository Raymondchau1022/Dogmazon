...
const Login = () => {
    ...
    const login = () =>{
        Axios.get(`http://localhost:5000/login_accounts/${emailReg}/${passwordReg}`
        ).then((response) => {
                ...
                <div className='loginForm'>  
                    <form id="loginform" onSubmit={checkLogin}>
                        ...
                        <div className='passwordLogin'>
                            <input type={mask ? "text" : "password"} placeholder="Password" className='loginPasswordField' onChange={(e) => {setPasswordReg(e.target.value);}}/>
                            <span className={mask ? "far fa-eye-slash" : "far fa-eye"} onClick={PasswordToggle}/>
                        </div>
                        <button className='submitLogin' onClick={checkLogin}>  Login  <i className="fa-solid fa-right-to-bracket"></i></button>
                        ...
                        <a className='goregister' href='/register'>Create Account</a>
                        <a className='goforgotpassword' href="/forgotpassword">Forgot Password</a>
                        </div>
                    </form>
                </div>
            </div> 
